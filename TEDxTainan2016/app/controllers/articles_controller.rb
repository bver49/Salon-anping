class ArticlesController < ApplicationController
  ...
  def index
    @article  = Article.where({status: "0",created_at: 60.days.ago..Time.now }).order("created_at DESC").limit(6)
  ...
  def show
    prepare_meta_tags(title: @article.title,
                      ...
                      og: { image: @article.thumb.url(:large),
                            description: @article.summary})
    @morearticles = Article.where("status = 0").tagged_with(@article.tag_list).order("created_at DESC").limit(4)
    @article.views+=1
    @article.save
  end
